...
public class DateDoubleMetricProcessor extends DoubleMetricProcessor {
	...
	boolean isPossuiPrimeiraMetrica = false;
	boolean isPossuiSegundaMetrica = false;
	...
	public DateDoubleMetricProcessor(String metric1, String metric2, boolean hasTreshold) {
		super(metric1, metric2, hasTreshold);
		initializeDates();

	}

	public DateDoubleMetricProcessor(boolean isPossuiPrimeiraMetrica, String metric1, String metric2, boolean hasTreshold, boolean printProjectName) {
		super(metric1, metric2, hasTreshold, printProjectName);

		this.isPossuiPrimeiraMetrica = isPossuiPrimeiraMetrica;
		initializeDates();
	...
	public void readMetrics() throws IOException {
		...
		if (this.isPossuiPrimeiraMetrica && !this.isPossuiSegundaMetrica) {
			this.readMetricsPossuiPrimeiraMetricaENaoPossuiaSegunda();

		} else if (!this.isOldRecent) {
			...
			for (ArrayList<File> projectVersions: getProjectVersions()) {
				...
				if (projectVersions.size() > 1) {
					...
					if (inLimit(1, projectVersions, getMetricsNames().get(0)) && (inLimit(1, projectVersions, getMetricsNames().get(1)))) {
						...
						fillGroupDateVersion(projectVersions, recentVersions, middleVersions, oldVersions);

						if (isValidProject(recentVersions, oldVersions) && inLimitLOC(1000, recentVersions)) {
							// if (isValidProject(recentVersions, oldVersions) && inLimitLOC(1000,
							...
							/*
							 * recentMetric1.add((((double)lastRecentMetricValue[0]/lastRecentMetricValue[2])*100000)-(((double)
							 * lastOldMetricValue[0]/lastOldMetricValue[2])*100000));
							 * recentMetric2.add((((double)lastRecentMetricValue[1]/lastRecentMetricValue
							 * [2])*100000)-(((double)lastOldMetricValue[1]/lastOldMetricValue[2])*100000));
							 * middleMetric1.add((((double)lastMiddleMetricValue
							 * [0]/lastMiddleMetricValue[2])*100000)-(((double)lastOldMetricValue[0]/lastOldMetricValue[2])*100000));
							 * middleMetric2
							 * .add((((double)lastMiddleMetricValue[1]/lastMiddleMetricValue[2])*100000)-(((double)lastOldMetricValue
							 * [1]/lastOldMetricValue[2])*100000));
							 * oldMetric1.add(((double)lastOldMetricValue[0]/lastOldMetricValue[2])*100000);
							 * oldMetric2.add(((double)lastOldMetricValue[1]/lastOldMetricValue[2])*100000);
							 */

							getProjectNames().add(
									lastRecentVersion.getParentFile().getParentFile().getName() + "/" + lastRecentVersion.getParentFile().getName());
	...
	public void readMetricsPossuiPrimeiraMetricaENaoPossuiaSegunda() throws IOException {

		for (ArrayList<File> projectVersions: getProjectVersions()) {

			if (projectVersions.size() > 0) {
				for (int i = projectVersions.size() - 1; i > 0; i--) {

					File versaoProjeto = projectVersions.get(i);

					// verifica se possui a primeira metrica
					if (isPossuiPrimeiraMetricaENaoPossuiSegunda(1, 1000, versaoProjeto, getMetricsNames().get(0), getMetricsNames().get(1))) {

						int[] lastRecentMetricValue = getMetricValue(versaoProjeto);

						recentMetric1.add((((double) lastRecentMetricValue[0] / lastRecentMetricValue[2]) * 100000));
						recentMetric2.add((((double) lastRecentMetricValue[1] / lastRecentMetricValue[2]) * 100000));

						getProjectNames().add(
								versaoProjeto.getParentFile().getParentFile().getName() + "/" + versaoProjeto.getParentFile().getName() + "/"
										+ versaoProjeto.getName());
			...
			if (projectVersions.size() > 1) {
				...
				if (inLimit(1, projectVersions, getMetricsNames().get(0)) && (inLimit(1, projectVersions, getMetricsNames().get(1)))) {
					...
					if (inLimitLOC(1000, projectVersions)) {
						...
						// aqui faz com 2 métricas, pegando o valor inicial menos final - CORRELACAO
						recentMetric1.add((((double) lastRecentMetricValue[0] / lastRecentMetricValue[2]) * 100000)
								- (((double) lastOldMetricValue[0] / lastOldMetricValue[2]) * 100000));
						recentMetric2.add((((double) lastRecentMetricValue[1] / lastRecentMetricValue[2]) * 100000)
								- (((double) lastOldMetricValue[1] / lastOldMetricValue[2]) * 100000));
						middleMetric1.add((((double) lastMiddleMetricValue[0] / lastMiddleMetricValue[2]) * 100000)
								- (((double) lastOldMetricValue[0] / lastOldMetricValue[2]) * 100000));
						middleMetric2.add((((double) lastMiddleMetricValue[1] / lastMiddleMetricValue[2]) * 100000)
								- (((double) lastOldMetricValue[1] / lastOldMetricValue[2]) * 100000));
	...
	private boolean isValidProject(ArrayList<File> recentVersions, ArrayList<File> middleVersions, ArrayList<File> oldVersions) {
	...
	private void fillGroupDateVersion(ArrayList<File> projectVersions, ArrayList<File> recentVersions, ArrayList<File> middleVersions,
	...
	private boolean isOldVersion(int year, int month) {
		...
		if (year == oldDates.get(0) || year == oldDates.get(1) || year == oldDates.get(2)) {
			return true;
		} else if (year == oldDates.get(3) && month <= 6) { return true; }
	...
	private boolean isRecentVersion(int year) {
		return year == recentDates.get(0) || year == recentDates.get(1) || year == recentDates.get(2) || year == recentDates.get(3);
	}
	...
	private boolean isRecentVersion(int year, int month) {
		...
		if (year == recentDates.get(1) || year == recentDates.get(2) || year == recentDates.get(3) || year == recentDates.get(4)) {
			return true;
		} else if (year == recentDates.get(0) && month > 6) { return true; }
